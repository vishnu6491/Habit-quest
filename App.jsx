...
const save = async (k,v)=>{ localStorage.setItem(k, JSON.stringify(v)); };
const load = async (k,fb)=>{ const r = localStorage.getItem(k); return r ? JSON.parse(r) : fb; };
...
const MAX_LIB = SHOP.reduce((s,i)=>s+i.lib,0); // 168
...
export default function App(){
  ...
  const S={
    ...
    card:(extra={})=>({background:"rgba(255,255,255,0.045)",borderRadius:20,border:"1px solid rgba(255,255,255,0.075)",backdropFilter:"blur(10px)"},extra),
            ...
            {group.map(h=>{
              ...
              return(
                <div key={h.id} onClick={()=>toggleHabit(h)} style={{
                  ...
                  border:`1.5px solid ${done?h.color+"55":"rgba(255,255,255,0.07)"}`,
                  transition:"all 0.2s",
                }}>
      ...
      <div style={{padding:"0 16px"}}>
        ...
        <div style={{position:"relative",height:280,margin:"0 auto",maxWidth:260,display:"flex",alignItems:"center",justifyContent:"center"}}>
          ...
          <div style={{
            ...
            background:`radial-gradient(ellipse at center, ${stage.glowR>0?"rgba(251,191,36,"+stage.auraOp*0.18+")":"transparent"} 0%, transparent 70%)`,
        ...
        <div style={{...S.card(),padding:"14px 18px",marginBottom:12}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
            ...
            <span style={{fontSize:11,color:"#64748b"}}>{xp} / {nextXP} XP</span>
          </div>
        ...
        <div style={{...S.card(),padding:"14px 18px",marginBottom:12}}>
          <div style={{fontSize:11,color:"#a78bfa",fontWeight:700,fontFamily:"'Cinzel',serif",letterSpacing:0.8,marginBottom:10}}>✦ Equipped Accessories</div>
          {equipped.length===0?(
            <div style={{textAlign:"center",color:"#374151",fontSize:13,padding:"10px 0"}}>No accessories equipped. Visit the Shop.</div>
          ):(
            <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
              {equipped.map(id=>{
                const item=SHOP.find(x=>x.id===id);
                return item?(
                  <div key={id} onClick={()=>toggleEquip(id)} style={{
                    display:"flex",alignItems:"center",gap:6,
                    background:"rgba(168,85,247,0.15)",borderRadius:10,
                    padding:"6px 12px",border:"1px solid rgba(168,85,247,0.3)",cursor:"pointer",
                  }}>
                ...
                <div key={i} style={{display:"flex",alignItems:"center",gap:10,position:"relative"}}>
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",width:20}}>
                    <div style={{width:16,height:16,borderRadius:"50%",flexShrink:0,
                      background:reached?"linear-gradient(135deg,#fbbf24,#f59e0b)":current?"linear-gradient(135deg,#a78bfa,#6366f1)":"rgba(255,255,255,0.07)",
                      border:current?"2px solid #a78bfa":"2px solid transparent",
                      boxShadow:current?"0 0 10px #a78bfa88":"none",
                    }}/>
                  ...
                  <div style={{flex:1,paddingBottom:i<STAGES.length-1?12:0}}>
                    <span style={{fontSize:13,fontWeight:current?700:500,color:reached?"#fbbf24":current?"#e2e8f0":"#374151",fontFamily:current?"'Cinzel',serif":undefined}}>
                      {s.title}
                    </span>
      ...
      {SHOP.map(item=>{
        ...
        return(
          <div key={item.id} style={{
            ...
            border:`1.5px solid ${eq?"rgba(168,85,247,0.4)":owned?"rgba(251,191,36,0.2)":"rgba(255,255,255,0.07)"}`,
            background:eq?"rgba(168,85,247,0.1)":owned?"rgba(251,191,36,0.05)":"rgba(255,255,255,0.04)",
          }}>
              ...
              return(
                <div key={i} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3}}>
                  <div style={{width:"100%",height:h,borderRadius:"5px 5px 0 0",
                    background:isToday?"linear-gradient(180deg,#a855f7,#6366f1)":m.col+"88",
                    boxShadow:isToday?"0 0 10px #a855f766":"none",transition:"height 0.6s"}}/>
    ...
    <div style={S.app}>
      ...
      <div style={{padding:"20px 18px 14px",background:"rgba(255,255,255,0.025)",borderBottom:"1px solid rgba(255,255,255,0.06)",backdropFilter:"blur(12px)",position:"sticky",top:0,zIndex:50}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <div>
            <div style={{fontSize:22,fontWeight:900,fontFamily:"'Cinzel',serif",
              background:"linear-gradient(90deg,#fbbf24,#a78bfa,#60a5fa)",backgroundSize:"200%",
              WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",
              animation:"shimmer 4s linear infinite"}}>HabitQuest</div>
      ...
      {/* Add Habit modal */}
      {showAdd&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:300,display:"flex",alignItems:"flex-end",backdropFilter:"blur(4px)"}}
